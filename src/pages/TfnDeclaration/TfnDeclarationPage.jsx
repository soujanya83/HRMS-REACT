...
const TfnDeclarationPage = () => {
  ...
  useEffect(() => {
    ...
    const currentRole = localStorage
      .getItem("CURRENT_USER_ROLE")
      ?.toLowerCase();
    ...
    setIsEmployeeUser(
      (hasEmployeeRole && !hasAdminRole) ||
        (!roleNames.length && !!employeeStr),
    );
  ...
  const fetchTfnDeclaration = async () => {
    try {
      ...
      const response = await axiosClient.get(
        `/tfn-declarations/employee/${employeeId}`,
      );
      ...
      if (response.data) {
        ...
        const updatedForm = {
          sectionA: {
            ...
            signatureDate: createCharArray(
              8,
              formatDateForInput(data.payee_declaration_date),
            ),
          },
          ...
          sectionB: {
            ...
            signatureDate: createCharArray(
              8,
              formatDateForInput(data.payer_declaration_date),
            ),
          },
        };
    ...
    try {
      ...
      if (declarationId) {
        ...
        response = await axiosClient.put(
          `/tfn-declarations/${declarationId}`,
          payload,
        );
        ...
        Object.keys(apiErrors).forEach((key) => {
          ...
          const errorMessages = {
            ...
            claim_tax_free_threshold:
              "Tax free threshold selection is required",
