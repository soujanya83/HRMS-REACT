...
const PayslipGeneration = () => {
        ...
        const updatedPayslips = payslips.map(payslip => 
            selectedPayslips.includes(payslip.id) && payslip.status === 'pending'
                ? { 
                    ...
                    payslip_url: `/payslips/${payslip.employee_id}_${new Date(payrollPeriod.year, payrollPeriod.month - 1).toLocaleString('default', { month: 'short', year: 'numeric' })}.pdf`
                }
                : payslip
        );
    ...
    const sendPayslips = () => {
        ...
        const updatedPayslips = payslips.map(payslip => 
            selectedPayslips.includes(payslip.id) && (payslip.status === 'generated' || payslip.status === 'pending')
                ? { 
                    ...payslip, 
                    status: 'sent', 
    ...
    const filteredPayslips = payslips.filter(payslip => {
        const matchesDepartment = filters.department === 'all' || payslip.department === filters.department;
        const matchesStatus = filters.status === 'all' || payslip.status === filters.status;
        const matchesSearch = filters.search === '' || 
                            payslip.employee_name.toLowerCase().includes(filters.search.toLowerCase()) ||
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-sm text-gray-600">
                                Payslips for: {new Date(payrollPeriod.year, payrollPeriod.month - 1).toLocaleString('default', { month: 'long', year: 'numeric' })}
                            </div>
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Payment Date</th>
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                ...
                                                {payslip.generated_date && (
                                                    <div className="text-xs text-gray-500 mt-1">
                                                        Generated: {new Date(payslip.generated_date).toLocaleDateString()}
                                                    </div>
                                                )}
                                            </td>
                        ...
                        {filteredPayslips.length === 0 && (
                            <div className="text-center py-12">
                                ...
                                <p className="text-gray-500">Generate payslips for the selected period.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {filteredPayslips.length > 0 && (
                    <div className="mt-4 bg-gray-50 px-4 py-3 border-t border-gray-200 rounded-lg">
                        <div className="flex justify-between items-center">
                            <div className="text-sm text-gray-600">
                                Showing {filteredPayslips.length} employees • 
                                Selected: {selectedPayslips.length} • 
                                Total Payout: {formatCurrency(
                                    filteredPayslips
                            ...
                            <div className="border-2 border-gray-300 p-6">
                                ...
                                <div className="text-center mb-6">
                                    ...
                                    <p className="text-sm text-gray-500">
                                        {new Date(payrollPeriod.year, payrollPeriod.month - 1).toLocaleString('default', { month: 'long', year: 'numeric' })}
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div>
                                        ...
                                        <p><strong>Name:</strong> {previewPayslip.employee_name}</p>
                                        <p><strong>ID:</strong> {previewPayslip.employee_id}</p>
                                        <p><strong>Department:</strong> {previewPayslip.department}</p>
                                        <p><strong>Position:</strong> {previewPayslip.position}</p>
                                    </div>
                                    <div>
                                        <h3 className="font-semibold mb-2">Payment Details</h3>
                                        <p><strong>Payment Date:</strong> {new Date(previewPayslip.payment_date).toLocaleDateString()}</p>
                                        <p><strong>Payment Method:</strong> {previewPayslip.payment_method}</p>
                                        <p><strong>Bank Account:</strong> {previewPayslip.bank_account}</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-6">
                                    {/* Earnings */}
                                    <div>
                                        <h3 className="font-semibold mb-2 border-b pb-1">Earnings</h3>
                                        <div className="space-y-1">
                                            <div className="flex justify-between">
                                                <span>Basic Salary</span>
                                                <span>{formatCurrency(previewPayslip.basic_salary)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                    ...
                                    {/* Deductions */}
                                    <div>
                                        <h3 className="font-semibold mb-2 border-b pb-1">Deductions</h3>
                                        <div className="space-y-1">
                                            <div className="flex justify-between">
                                ...
                                <div className="bg-gray-100 p-4 rounded text-center">
                                    <h3 className="font-semibold text-lg">Net Salary: {formatCurrency(previewPayslip.net_salary)}</h3>
                                    <p className="text-sm text-gray-600">In Words: {convertToWords(previewPayslip.net_salary)}</p>
                                </div>
                            </div>
