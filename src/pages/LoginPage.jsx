...
const LoginPage = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const superAdminOrg = userRoles.find(
          (r) => r.role_name?.toLowerCase() === "superadmin",
        );
        if (superAdminOrg) {
          selectedOrg = superAdminOrg;
          selectedRole = superAdminOrg.role_name;
          console.log(
            "Found superadmin organization:",
            superAdminOrg.organization_name,
          );
        } else {
  ...
  const validateForgotPasswordForm = () => {
    ...
    if (!resetEmail.trim()) newErrors.resetEmail = "Email is required.";
      ...
      newErrors.resetEmail = "Email address is invalid.";
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
    ...
    if (validateForgotPasswordForm()) {
      ...
      try {
        ...
        if (response.data.status === true) {
          setSuccessMessage(
            response.data.message || "Password reset OTP sent to email",
          );
            ...
            {!showForgotPassword &&
              ...
              !showResetPassword && (
                <p className="mt-6 text-blue-200">
                  CHRISPP is an acronym for Cloud-based Human Resource
                  Information System with Payroll and Performance Management. A
                  smart, easy, cloud-based HR for people who matter most.
                </p>
              )}
            {(showForgotPassword ||
              showOtpVerification ||
          ...
          <div className="w-full max-w-md">
              ...
              !showResetPassword && (
                <>
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    {apiError && (
                      <p className="text-red-500 text-sm text-center mb-4 bg-red-100 p-3 rounded-lg">
                        {apiError}
                      </p>
                    )}
                    <div className="mb-6">
            ...
            {showForgotPassword && (
              <>
                ...
                {successMessage && (
                  <p className="text-green-500 text-sm text-center mb-4 bg-green-100 p-3 rounded-lg">
                    {successMessage}
                  </p>
                )}
