...
export default function EmployeeList() {
    ...
    const handleDelete = (id) => {
        openModal(() => deleteEmployee(id), 'Confirm Trash', 'Are you sure you want to move this employee to the trash?');
    };

    const handleRestore = (id) => {
        openModal(() => restoreEmployee(id), 'Confirm Restore', 'Are you sure you want to restore this employee?');
    };

    const handleForceDelete = (id) => {
        openModal(() => forceDeleteEmployee(id), 'Confirm Permanent Deletion', 'This action is irreversible. Are you sure you want to permanently delete this employee and all associated data?');
    };
        ...
        <div className="p-4 md:p-6 lg:p-8 bg-gray-100 min-h-full font-sans">
            ...
            <div className="mb-6 p-4 bg-white shadow-lg rounded-lg flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-2 p-1 bg-gray-200 rounded-lg">
                    <button onClick={() => setView('active')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${view === 'active' ? 'bg-white shadow text-blue-600' : 'text-gray-600'}`}>Active</button>
                    <button onClick={() => setView('trashed')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${view === 'trashed' ? 'bg-white shadow text-blue-600' : 'text-gray-600'}`}>Trash</button>
                </div>
            </div>
                        ...
                        {loading ? (
                            ...
                            employees.map((emp) => (
                                <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${emp.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                            {emp.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div className="flex justify-center items-center gap-4">
                                            {view === 'active' ? (
                                                <>
                                                    <Link to={`/dashboard/employees/${emp.id}`} title="View Profile" className="text-blue-600 hover:text-blue-800 transition-colors"><FaEye size={16} /></Link>
                                                    <Link to={`/dashboard/employees/edit/${emp.id}`} title="Edit" className="text-yellow-500 hover:text-yellow-700 transition-colors"><FaEdit size={16} /></Link>
                                                    <button onClick={() => handleDelete(emp.id)} title="Trash" className="text-red-600 hover:text-red-800 transition-colors"><FaTrash size={16} /></button>
                                                </>
                                            ) : (
                                                <>
                                                    <button onClick={() => handleRestore(emp.id)} title="Restore" className="text-green-600 hover:text-green-800 transition-colors"><FaUndo size={16} /></button>
                                                    <button onClick={() => handleForceDelete(emp.id)} title="Delete Permanently" className="text-red-800 hover:text-red-900 transition-colors"><FaTrashAlt size={16} /></button>
                                                </>
                                            )}
                                        </div>
                                    </td>
