...
const ShiftSwapping = () => {
  ...
  const [newRequest, setNewRequest] = useState({
    ...
    swap_shift_date: new Date(Date.now() + 86400000)
      .toISOString()
      .split("T")[0],
  });
  ...
  const handleSubmitRequest = (e) => {
    ...
    const request = {
      ...
      requester_shift_name:
        shifts.find((s) => s.id === parseInt(newRequest.requester_shift_id))
          ?.name || "",
      requester_shift_date: newRequest.requester_shift_date,
      swap_with_id: newRequest.swap_with_id,
      swap_with_name:
        employees.find((emp) => emp.id === parseInt(newRequest.swap_with_id))
          ?.name || "",
      swap_shift_id: newRequest.swap_shift_id,
      swap_shift_name:
        shifts.find((s) => s.id === parseInt(newRequest.swap_shift_id))?.name ||
        "",
  ...
  const filteredRequests = swapRequests.filter((request) => {
    ...
    const matchesDepartment =
      ...
      employees.find((emp) => emp.id === request.requester_id)?.department ===
        filters.department;
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 mt-4 rounded-lg">
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-600">
                Showing {filteredRequests.length} of {swapRequests.length}{" "}
                requests
              </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <p className="text-sm">
                        <strong>Shift:</strong>{" "}
                        {selectedRequest.requester_shift_name}
                      </p>
                      <p className="text-sm">
                        <strong>Date:</strong>{" "}
                        {selectedRequest.requester_shift_date}
                      </p>
                    </div>
                    ...
                    <div className="bg-green-50 p-4 rounded-lg">
                      ...
                      <p className="text-sm">
                        <strong>Employee:</strong>{" "}
                        {selectedRequest.swap_with_name}
                      </p>
                      <p className="text-sm">
                        <strong>Shift:</strong>{" "}
                        {selectedRequest.swap_shift_name}
                      </p>
                      <p className="text-sm">
                        <strong>Date:</strong> {selectedRequest.swap_shift_date}
                      </p>
                    </div>
                  </div>
