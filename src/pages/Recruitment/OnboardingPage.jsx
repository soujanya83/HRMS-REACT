          ...
          try {
            ...
            return {
              ...
              applicant: {
                ...
                job_opening: applicant.job_opening || { title: 'Not specified' }
              },
...
const NewHireCard = ({ hire, onSelect }) => {
  ...
  return (
    <div 
      onClick={onSelect} 
      className="bg-white rounded-xl shadow-md p-6 cursor-pointer transition transform hover:-translate-y-1 hover:shadow-lg"
    >
...
const TemplateManager = () => {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      const templatesRes = await getOnboardingTemplates({ organization_id: organizationId });
...
// Updated New Hire Checklist SlideOver with API Integration
const NewHireChecklistSlideOver = ({ hire, isOpen, onClose, onTaskUpdate }) => {
                  ...
                  tasks.map(task => (
                    <div key={task.id} className="flex items-center">
                      ...
                      <label 
                        ...
                        className={`ml-3 text-sm ${task.status === 'completed' ? 'text-gray-500 line-through' : 'text-gray-800'}`}
                      >
