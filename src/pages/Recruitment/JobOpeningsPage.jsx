...
import { HiPlus, HiPencil, HiTrash, HiArrowLeft, HiOutlineBriefcase, HiOutlineOfficeBuilding, HiOutlineLocationMarker, HiOutlineClock, HiX } from 'react-icons/hi';
import { useOrganizations } from '../../contexts/OrganizationContext';
import { 
    getJobOpenings, createJobOpening, updateJobOpening, deleteJobOpening, getJobOpeningById,
        ...
        <div className="p-4 sm:p-6 lg:p-8 font-sans bg-gray-50 min-h-full">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                    ...
                    <button onClick={() => { setEditingJob(null); setModalErrors(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-brand-blue text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:opacity-90 transition self-start sm:self-center">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {jobs.map(job => (
                                <JobCard 
                                    key={job.id} 
                                    job={job} 
                                    onEdit={() => { setEditingJob(job); setModalErrors(null); setIsModalOpen(true); }}
                                    onDelete={() => { setJobToDelete(job); setIsConfirmOpen(true); }}
                                />
                            ))}
                        </div>
                    ) : (
            ...
            <JobOpeningModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} job={editingJob} errors={modalErrors} />
            <ConfirmationModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={handleDelete} title="Delete Job Opening" message={`Are you sure you want to delete "${jobToDelete?.title}"?`} />
        </div>
    );
}
            ...
            <div className="p-6">
                <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                    <div className="flex-1">
                        <Link to={`/dashboard/recruitment/jobs/${job.id}`} className="block">
                            <h2 className="text-xl font-bold text-gray-900 hover:text-brand-blue transition truncate">{job.title}</h2>
                        </Link>
                        <div className="flex items-center flex-wrap gap-x-4 gap-y-2 mt-2 text-sm text-gray-500">
                            <span className="flex items-center gap-1.5"><HiOutlineOfficeBuilding />{job.organization.name}</span>
                            <span className="flex items-center gap-1.5"><HiOutlineLocationMarker />{job.location}</span>
                            <span className="flex items-center gap-1.5"><HiOutlineClock />{job.employment_type}</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 self-start sm:self-center flex-shrink-0">
                        <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusClasses[job.status] || 'bg-gray-100 text-gray-800'}`}>
                            {job.status}
                        </span>
                        <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={onEdit} className="p-2 text-gray-500 hover:bg-gray-200 rounded-full"><HiPencil /></button>
                            <button onClick={onDelete} className="p-2 text-gray-500 hover:bg-red-100 hover:text-red-600 rounded-full"><HiTrash /></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 font-sans bg-gray-50 min-h-full">
            <div className="max-w-4xl mx-auto">
                <button onClick={() => navigate("/dashboard/recruitment/jobs")} className="flex items-center gap-2 text-brand-blue hover:underline mb-4 font-semibold">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
                    ...
                    <div className="mt-6 text-sm text-gray-500">
                        <p>Posting Date: {new Date(job.posting_date).toLocaleDateString()}</p>
...
function JobOpeningModal({ isOpen, onClose, onSave, job, errors }) {
    ...
    useEffect(() => {
        const initialData = job || { title: '', location: '', employment_type: 'Full-time', status: 'Open', description: '', requirements: '', posting_date: new Date().toISOString().split('T')[0], closing_date: '', department_id: '', designation_id: '' };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-2xl font-bold text-gray-800">{job ? 'Edit Job Opening' : 'Create New Job Opening'}</h2>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
                    ...
                    <div className="flex justify-end gap-4 p-6 border-t border-gray-200 bg-gray-50">
                        <button type="button" onClick={onClose} className="py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">Cancel</button>
                        <button type="submit" className="py-2 px-4 bg-brand-blue text-white font-semibold rounded-lg hover:opacity-90 transition">{job ? 'Save Changes' : 'Create Job'}</button>
                    </div>
                </form>
            </div>
        </div>
